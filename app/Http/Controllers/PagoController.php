    ...
    public function exito(Request $request)
{
    ...
    // 1) Verificamos que sea la misma sesión de Stripe que guardamos antes
    if (
        !$sessionId ||
    ...
    // 2) Recuperamos detalles de Stripe y comprobamos pago
    ...
    // 3) Leemos de la sesión los datos que guardamos antes en prepararPago()
    ...
    foreach ($pasajerosDatos as $p) {
        // (A) CREAR billete de ida si existe asiento_ida
        if (!empty($p['asiento_ida'])) {
            ...
            $billeteIda = Billete::create([
                ...
                'cancelacion_flexible'  => $cancelacionFlexibleGlobal, // se lee de sesión
        ...
        // (B) CREAR billete de vuelta si existe asiento_vuelta
        if (!empty($p['asiento_vuelta'])) {
            ...
            $rutaCompletaQRVuelta = storage_path(
                'app/public/' . $nombreArchivoQRVuelta
            );
            File::put(
    ...
    // (C) Recargamos billetes para el correo y la vista
    $billetes = Billete::whereIn(
            'id',
            collect($billetesCreados)->pluck('id')
        )
